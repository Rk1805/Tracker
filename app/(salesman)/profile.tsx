...
export default function SalesmanProfile() {
  ...
  useEffect(() => {
    ...
    const tripsUnsub = onSnapshot(
      ...
      (snapshot) => {
        let visits = 0, trips = 0;
        snapshot.forEach((doc) => { trips++; visits += doc.data().completedStops || 0; });
        setStats((s) => ({ ...s, totalVisits: visits, totalTrips: trips }));
      }
    );

    const leadsUnsub = onSnapshot(
      query(collection(firebaseService.firestore, 'leads'), where('createdBy', '==', appUser.uid)),
      (snapshot) => {
        let total = 0, approved = 0;
        snapshot.forEach((doc) => { total++; if (doc.data().status === 'approved') approved++; });
        setStats((s) => ({ ...s, totalLeads: total, approvedLeads: approved }));
      }
    );

    return () => { tripsUnsub(); leadsUnsub(); };
  }, [appUser?.uid]);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.profileHeader}>
        <View style={styles.avatarPlaceholder}>
          <Text style={styles.avatarText}>
            {appUser?.displayName?.charAt(0)?.toUpperCase() || 'S'}
          </Text>
        </View>
      ...
      <View style={styles.statsGrid}>
        <View style={styles.statCard}>
          ...
          <Text style={styles.statLabel}>Parties Visited</Text>
        </View>
        <View style={styles.statCard}>
        ...
        <View style={styles.statCard}>
          <Text style={styles.statValue}>{stats.approvedLeads}</Text>
...
const styles = StyleSheet.create({
  ...
  statValue: { fontSize: 28, fontWeight: '700', color: '#34C759' },
