...
export default function TripsScreen() {
  ...
  const handleReorderStops = async (tripId: string, stopIndex: number, direction: 'up' | 'down') => {
    ...
    if (stops[targetIndex].status !== 'pending' || stops[stopIndex].status !== 'pending') return;
                    ...
                    <View style={styles.stopMainRow}>
                      ...
                      {isActive && isPending && (
                        <View style={styles.orderControls}>
                          <TouchableOpacity onPress={() => handleReorderStops(item.id, index, 'up')} style={styles.orderBtn}>
                            <Text style={styles.orderBtnText}>▲</Text>
                          </TouchableOpacity>
                          <TouchableOpacity onPress={() => handleReorderStops(item.id, index, 'down')} style={styles.orderBtn}>
                            <Text style={styles.orderBtnText}>▼</Text>
                          </TouchableOpacity>
                        </View>
                      )}
            ...
            <View style={styles.topNavOverlay}>
              <View style={styles.navHeaderCard}>
                <Text style={styles.navDirectionText}>
                  Next Action: Head {modalNavInfo.bearingDirection} to <Text style={{fontWeight: '700'}}>{modalNavInfo.nextStop?.partyName}</Text>
                </Text>
...
const styles = StyleSheet.create({
  ...
  stopContainerItem: { backgroundColor: '#F9F9F9', padding: 10, borderRadius: 8, marginVertical: 2 },
